function out = solve_discrete_tnn_penalty(A0, B, par)
%#ok<*EQEFF>
%#ok<*VUNUS>
...
hst0.gammas = logspace(par.s.min_gam, par.s.max_gam, par.s.num_gam );

stop_all = 0;

for gk = 1:par.s.num_gam
    
    if stop_all == 0
        
        gamma = hst0.gammas(gk);
        
        fprintf('\n=============== Gamma %1.3e (%d of %d) ================= \n\n', ...
            gamma, gk, par.s.num_gam);
        ...
        for k=1:par.m.MAX_ITER
            ...
            opt = cvx_tnn(A0, B, K1, K2, gamma, par);
            ...
            if k > 1
                ...
                if no_dec_ctr >= par.m.max_no_decrease
                    warning('>>>>> LOW DESCREASE between iterations. STOPPED.\n')
                    stop_all = 1;
                    break
                end
